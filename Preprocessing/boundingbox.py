#Oushesh Haradhun 
#Programming goes in the binary Ground Truth image Label
#find the bounding box for each ground truth label, crop the ROI out 
#and saves it + the edge coordinates stacked onto a big array. Then use
#this array of ccordinates and crop the corresponding image from the RGB image

import cv2
import numpy as np

QR_orig = cv2.imread('29.tif', 0)
QR = cv2.imread('p29.tif', 0) # read the QR code binary image as grayscale image to make sure only one layer
