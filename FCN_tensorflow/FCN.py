...
IMAGE_SIZE = 224 #to be used for resizing for VGG network
IMAGE_WIDTH = 966
IMAGE_HEIGTH = 1225

#Load Image here:
dirnametrain_records = 'images/training/'
dirnamevalid_records = 'images/validation/'
#labels
dirnametrain_annotation = 'annotations/training/'
dirnamevalid_annotation = 'annotations/validation/'
...
#Read the Training Images as train_records
for fname in os.listdir(dirnametrain_records): #Filename of the individual images
    im = Image.open(os.path.join(dirnametrain_records, fname)) # Concateneate the directory name + Filenmae into 1 string
    imarray = np.array(im)
...
#Read the Training labels as train_annotation
for fname in os.listdir(dirnametrain_annotation): #Filename of the individual images
    im = Image.open(os.path.join(dirnametrain_annotation,fname)) # Concateneate the directory name + Filenmae into 1 string
    imarray = np.array(im)
    train_labels.append(imarray)    

train_labels =  np.asarray(train_labels)
print (train_labels.shape)

#Read the Validation images as Validation_records
for fname in os.listdir(dirnamevalid_records): #Filename of the individual images
    im = Image.open(os.path.join(dirnamevalid_records , fname)) # Concateneate the directory name + Filenmae into 1 string
    imarray = np.array(im)
    valid_data.append(imarray)    

valid_data =  np.asarray(valid_data)
print (valid_data.shape)

#Read the validation_labels as validation_annotations
for fname in os.listdir(dirnamevalid_annotation): #Filename of the individual images
    im = Image.open(os.path.join(dirnamevalid_annotation , fname)) # Concateneate the directory name + Filenmae into 1 string
    imarray = np.array(im)
    valid_labels.append(imarray)    
...
def inference(image, keep_prob):
    ...
    :return:
    """
    print("setting up vgg initialized conv layers ...") #TILL noW everything is 
...
def main(argv=None):
    ...
    for idx in range(train_data.shape[0]):
        resized= scipy.ndimage.interpolation.zoom(train_data[idx], (224/966,224/1225,1.0)) #966 to 224 factor of 
        ...
        resized_labels = scipy.ndimage.interpolation.zoom(train_labels[idx], (224/966,224/1225)) #966 to 224 factor of 
    ...
    for idx in range(valid_data.shape[0]):    
        valid_resized = scipy.ndimage.interpolation.zoom(valid_data[idx], (224/966,224/1225,1.0)) #966 to 224 factor of 
        resized_valid_data.append(valid_resized) 
        resized_labels_array = scipy.ndimage.interpolation.zoom(valid_labels[idx], (224/966,224/1225)) #966 to 224 factor of 
    ...
    resized_valid_labels = np.asarray(resized_valid_labels) #convert to np.array    
